...
class ConnectionManager:
    ...
    async def disconnect(self, username: str, websocket: WebSocket):
        ...
        if len(self.active_connections[username]) == 0:
            ...
            await self.remove_user_from_rooms(username)
    ...
    async def send_private_message(self, receiver: str, data: dict):

        connections = self.active_connections.get(receiver, set())
    ...
    async def broadcast_status(self, username, status):
    ...
    async def join_call_room(self, room, username):

        if room not in self.call_rooms:
            self.call_rooms[room] = set()

        self.call_rooms[room].add(username)

        print(username, "joined", room)


    async def leave_call_room(self, room, username):

        if room in self.call_rooms:

            self.call_rooms[room].discard(username)

            if len(self.call_rooms[room]) == 0:
                del self.call_rooms[room]


    async def remove_user_from_rooms(self, username):
    ...
    async def broadcast_call_signal(
        ...
        users = self.call_rooms.get(room, set())
